...
また,CFRPのパイプが高価であることやCFRPの加工が困難であること,パイプが固定し難いことが問題となった.
...
機体の姿勢制御を行うことを考慮すると,総重量を最大推力の？割以下にしたいと考える. %何割にしましょうか
...
ルンゲ・クッタ法とは,数値解析において微分方程式の初期値問題に対して近似解を与える一連の方法である.\cite{runge} %wikiのコピペなう
...
この状態でモータに与えるPWM値を0.05[ms]操作し,PWM値と推力の関係を求める.
実験により得られたグラフからモータの始動から推力の上限に達するまでの範囲を抜き出し,その近似直線をモータの制御式として用いる.
...
この状態で1つのモータのPWM値を操作し,機体が1回転するのにかかる時間を計測する.
実験により得られたグラフから機体が回転し始める点から最短時間で1回転する点までを抜き出し,その近似直線をモータの制御式として用いる.
